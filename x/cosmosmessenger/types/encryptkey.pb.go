...
// source: cosmosmessenger/cosmosmessenger/encryptkey.proto
...
func init() {
	proto.RegisterFile("cosmosmessenger/cosmosmessenger/encryptkey.proto", fileDescriptor_bc8573d1a71d401b)
}
...
func encodeVarintEncryptkey(dAtA []byte, offset int, v uint64) int {
	offset -= sovEncryptkey(v)
	base := offset
	for v >= 1<<7 {
...
func sovEncryptkey(x uint64) (n int) {
...
func (m *Encryptkey) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		default:
			iNdEx = preIndex
			skippy, err := skipEncryptkey(dAtA[iNdEx:])
			if err != nil {
				return err
			}
...
func skipEncryptkey(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	depth := 0
	for iNdEx < l {
		...
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowEncryptkey
			}
			if iNdEx >= l {
		...
		case 2:
			...
			if length < 0 {
				return 0, ErrInvalidLengthEncryptkey
			}
			iNdEx += length
		case 3:
			depth++
		case 4:
			if depth == 0 {
				return 0, ErrUnexpectedEndOfGroupEncryptkey
			}
			depth--
		case 5:
			iNdEx += 4
...
var (
	ErrInvalidLengthEncryptkey        = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowEncryptkey          = fmt.Errorf("proto: integer overflow")
	ErrUnexpectedEndOfGroupEncryptkey = fmt.Errorf("proto: unexpected end of group")
)
